...
/// What the engine can do, for callers that cannot be `@available`-gated themselves.
/// An empty list is the one answer both a Mac without the API and a Mac without the
/// models can give, and it means the same thing to whoever asked: pick another engine.
public enum AppleSpeech {
    public static func languages() async -> [String] {
        guard #available(macOS 26, *), AppleTranscriber.isAvailable else { return [] }
        return await AppleTranscriber.supportedLanguages()
    }

    public static func describe(_ languages: [String]) -> String {
        languages
